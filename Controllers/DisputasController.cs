        ...
        public async Task<IActionResult> DisputaEmGrupoAsync(Disputa d)
        {
            try
            {
                ...
                List<Personagem> personagens = await _context.TB_PERSONAGENS
                    .Include(p => p.Arma)
                    .Include(p => p.PersonagemHabilidades).ThenInclude(Ph => Ph.Habilidade)
                    .Where(p => d.ListIdPersonagens.Contains(p.Id))
                    .ToListAsync();
                ...
                while (qtdPersonagensVivos > 1)
                {
                    ...
                    else if (atacante.PersonagemHabilidades.Count != 0)//Verificar se o personagem tem habilidades
                    {
                        //Programação do ataque com habilidade

                        //Realiza o sorteio entra as habilidades existentes e na linha seguinte a seleciona.
                    ...
                    {
                        //Incrementa os dados dos combatentes
                        atacante.Vitorias++;
                        oponente.Derrotas++;
                        atacante.Disputas++;
                        oponente.Disputas++;

                        d.Id = 0;//Zera o Id para poder salvar os danos de disputa sem erro de chave.
                        d.DataDisputa = DateTime.Now;
                        _context.TB_DISPUTAS.Add(d);
                        await _context.SaveChangesAsync();
                    }
